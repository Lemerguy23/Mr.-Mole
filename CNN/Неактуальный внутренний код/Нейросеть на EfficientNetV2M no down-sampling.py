...
COEF_FOTO = 15
COEF_POSITIVE = 1
parametrs = "model_EfficientNetV2M_not_full150_Dp_5,3_VeryBigD_Flips_notManyDT_albumentationsAndAM"
...
def smart_cutmix(batch_x, batch_y, p=0.5):
    ...
    def apply_cutmix(i, out_x):
        ...
        def _mix():
            # находим все индексы с тем же классом
            same_cls = tf.where(batch_y == batch_y[i])  # форма [K,1]
            same_cls = tf.reshape(same_cls, [-1])       # [K]

            # убираем сам i
            mask = same_cls != tf.cast(i, tf.int64)
            same_cls = tf.boolean_mask(same_cls, mask)
            ...
            def _with_candidate():
                ...
                # Обновляем элемент i
                return tf.tensor_scatter_nd_update(out_x, [[i]], [img2_aug])
...
# После разделения на train_paths и val_paths
train_labels_np = np.array(train_labels)
num_class0 = np.sum(train_labels_np == 0)
num_class1 = np.sum(train_labels_np == 1)

# Создаем датасеты для каждого класса из обучающей выборки
ds_class0_train = tf.data.Dataset.from_tensor_slices(
    (np.array(train_paths)[train_labels_np == 0], 
    np.array(train_labels)[train_labels_np == 0])
)
ds_class1_train = tf.data.Dataset.from_tensor_slices(
    (np.array(train_paths)[train_labels_np == 1], 
    np.array(train_labels)[train_labels_np == 1])
)

train_balanced = tf.data.experimental.sample_from_datasets(
    [ds_class0_train, ds_class1_train],
    weights=[0.5, 0.5]  # Равная вероятность выборки из каждого класса
).repeat()

# Ограничиваем размер датасета
steps_per_epoch = min(num_class0, num_class1) * 2 // BATCH_SIZE
train_balanced = train_balanced.take(steps_per_epoch * BATCH_SIZE)

# Создаем итоговый train_ds
train_ds = (
    train_balanced
...
print(f"Количество элементов в train_ds: {tf.data.experimental.cardinality(train_ds).numpy() * BATCH_SIZE}")
