...
COEF_POSITIVE = 1
COEF_NEGATIVE = 3
parametrs = "model_SSL_MobileNetV3Small_Dp_2,4_VeryBigD_224_notManyDT_albumentations_Coef15,1"
PATH_TO_SSL_MODEL = "/mnt/c/Users/urako/OneDrive/Документы/Code/Mr.-Mole/CNN/ssl_mole_encoder_from_ckpt62.keras"
...
def apply_albumentations(image, label):
    def _aug(img_np):
        return albumentations_transform(image=img_np)['image']
    aug_uint8 = tf.numpy_function(_aug, [image], tf.uint8)
    aug_uint8.set_shape([*IMG_WIDTH, IMG_CHANNELS])
    return aug_uint8, label
...
print(f"Загрузка SSL-предобученной модели из: {PATH_TO_SSL_MODEL}") 
try: 
    base_model = keras.models.load_model(PATH_TO_SSL_MODEL, compile=False, safe_mode=False ) 
    base_model._name = "ssl_mobilenetv3small_encoder" 
    print(f"SSL модель '{base_model.name}' успешно загружена.") 
except Exception as e: 
    print(f"ОШИБКА: Не удалось загрузить SSL модель из {PATH_TO_SSL_MODEL}. Ошибка: {e}") 
    exit()
...
output_tensor = layers.Dense(1, activation='sigmoid', name='class_predictions', dtype='float32')(x)
